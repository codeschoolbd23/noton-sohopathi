  ...
  const amarclass=useLoaderData()
    ...
    return (
      <div className="p-5 flex flex-col justify-center items-center">
        ...
        <p>{userClass?.userClassName} Question Bank is bellow.</p>
        <div className="card image-full w-[100%] lg:w-[500] shadow-sm mx-auto">
          <ul className="list">
            {amarclass.map((amar) => (
              (userClass?.userClassPosition===amar?.userClass) && <div key={amar._id}>
                <li className="list-row">
                  ...
                  <div className="list-col-grow">
                    <div>{amar.bookName}</div>
                    <div className="text-xs uppercase font-semibold opacity-60">
                      {amar.author}
                    </div>
                  </div>
                </li>
                <div>{amar.session}</div>
              </div>
            ))}
          </ul>
        </div>
